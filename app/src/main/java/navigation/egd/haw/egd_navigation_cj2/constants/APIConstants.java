package navigation.egd.haw.egd_navigation_cj2.constants;

/**
 * @author prannoy
 * @desciption The constants related with the APIs
 * Created by prann on 10/20/2017.
 */

public class APIConstants {
    public APIConstants(){}

    //TODO: replace with Real API KEYS
    public static String GOOGLE_DIRECTIONS_API_KEY = "AIzaSyBPVLp0tiS_bvL3OCRS1Mnwq_WiLW5myhE";
    public static String GOOGLE_DIRECTIONS_API_BASE_URL = "https://maps.googleapis.com/";
    public static String MODE_WALKING= "walking";
    public static int READ_TIMEOUT_MILLI_SEC = 1000;
    public static int WRITE_TIMEOUT_MILLI_SEC = 1000;
    public static int CONNECT_TIMEOUT_MILLI_SEC = 1000;
}
